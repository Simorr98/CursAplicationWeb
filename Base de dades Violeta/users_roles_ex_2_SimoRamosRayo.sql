...
GRANT SHOW DATABASES ON *.* TO 'testing_user'@'localhost'; -- show databases:
-- 4.5 Poder llistar totes les taules :
-- No hi ha cap privilegi especific per a delimitar aquesta acció. Quan se té accés a una bbdd,
-- se té accés a mostrar totes les seves taules
...
-- revoke grant option:
REVOKE GRANT OPTION ON *.* TO 'testing_user'@'localhost'; -- lleva grant option de totes ses bbdd i totes ses taules:
